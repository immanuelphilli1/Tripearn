...
const ProfileCard = ({handleDetails, arrival, departure, price, packageID, date, id, status}) => {
    return (
        <div className=" bg-light_black border border-white p-4 md:p-10 rounded-lg w-full">
            <div className="flex flex-col md:flex-row justify-between gap-10">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-1 lg:text-lg"><span>Status:</span><span className={`font-bold ${status === "active" ? "text-green" : status === "pending" ? " text-orange" :status === "inactive" ? " text-red" :status === "awaiting payment" ? " text-blue" : "text-purple" }  uppercase`}>{status}<span className="hidden">{id}</span></span></div>
